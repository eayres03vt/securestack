...
| CKV_AWS_103, CKV_AWS_2, CKV2_AWS_20 | ALB should use TLS 1.2 / HTTPS / redirect HTTP to HTTPS | Requires an ACM TLS certificate, which requires a domain name you actually own - AWS won't issue a certificate for the load balancer's default `*.elb.amazonaws.com` address. This project doesn't currently own a custom domain. Adding one (and HTTPS) is a natural next step if this were pointed at a real domain |
