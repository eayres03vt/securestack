...
| No CSRF protection on the add/edit/delete forms | A malicious site could embed a hidden form targeting e.g. `/delete/1` and submit it using a logged-in visitor's own session | Flask-WTF `CSRFProtect` + a token in every form |
...
**Known remaining limitation:** the ALB still serves plain HTTP (see the TLS exception below), so the login form and session cookie travel unencrypted between the browser and the load balancer, even though a login is now required. The session cookie is set with `HttpOnly` and `SameSite=Lax` to reduce theft/CSRF risk in the meantime, but `Secure` can't be enabled until there's real HTTPS in front of the app - enabling it today would make the browser refuse to send the cookie at all. Adding a real domain + free ACM certificate would close this fully — see the README's "what I'd do differently at scale" section.
